...
`row_gap` は、Agent パネルと Space パネルごとに、エントリ間の空白行数を指定します。デフォルトは `0` で、エントリを詰めて表示します。以前の間隔に戻すには `1` に設定します。`rows` で定義したコンテンツ行の間隔には影響しません。Space パネルでは、`row_gap` は完全な Space エントリ間にのみ適用されます。Space グループ内のネストされた worktree の子には代わりに `worktree_row_gap` が適用され、デフォルトは `0` なのでグループは詰めて表示されます。
