...
`row_gap` 分别控制 Agent 和 Space 面板中条目之间的空白终端行数。默认值为 `0`，会紧密排列条目；设为 `1` 可恢复之前的间距。它不会在 `rows` 声明的内容行之间添加间距。在 Space 面板中，`row_gap` 只作用于完整的 Space 条目之间；Space 组内嵌套的 worktree 子项改用 `worktree_row_gap`，其默认值为 `0`，因此组内条目保持紧密排列。
